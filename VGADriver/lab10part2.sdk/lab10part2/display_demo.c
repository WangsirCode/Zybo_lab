...
unsigned char RD_own(int i,int j){
	return (i-DIM)^2+(j-DIM)^2;
}
unsigned char GR_own(int i,int j){
	return j^j-i^i;
}
unsigned char BL_own(int i,int j){
  return i^i-j^j;
}
...
//				}
				...
				fBlue = BL(xcoi,ycoi);
				fGreen = GR(xcoi,ycoi);
				fColor =  ((u32) fRed << BIT_DISPLAY_RED) | ((u32) fBlue << BIT_DISPLAY_BLUE) | ( (u32) fGreen << BIT_DISPLAY_GREEN);
				frame[iPixelAddr] = fColor;
				/*
...
//		Xil_DCacheFlushRange((unsigned int) frame, DISPLAYDEMO_MAX_FRAME * 4);
		...
		for(xcoi = 0; xcoi < width; xcoi++)
		{
			...
			for(ycoi = 0; ycoi < height; ycoi++)
			{
				fRed = RD_own(xcoi,ycoi);
				fBlue = BL_own(xcoi,ycoi);
				fGreen = GR_own(xcoi,ycoi);
				fColor =  ((u32) fRed << BIT_DISPLAY_RED) | ((u32) fBlue << BIT_DISPLAY_BLUE) | ( (u32) fGreen << BIT_DISPLAY_GREEN);
				frame[iPixelAddr] = fColor;
				/*
				 * This pattern is printed one vertical line at a time, so the address must be incremented
